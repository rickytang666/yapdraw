...
export default function LibraryView() {
  ...
  useKeyboardShortcuts({
    ...
    'mod+n': () => { if (!isTrash) setShowNewModal(true) },
    'escape': () => { lib.clearSelection() },
  })
  ...
  function handleDragOver(event: DragOverEvent) {
    const { over } = event
    setOverFolderId(over && typeof over.id === 'string' && over.id.startsWith('folder:')
      ? over.id.slice(7)
      : null
    )
  }
    ...
    switch (lib.state.activeSection) {
      ...
      default:
        if (lib.state.activeSection.startsWith('folder:')) {
          const folder = folderHook.folders.find(f => f.id === lib.state.activeSection.slice(7))
  ...
  return (
    <DndContext sensors={sensors} onDragOver={handleDragOver} onDragEnd={handleDragEnd}>
      <div className="flex h-screen w-screen overflow-hidden bg-background">
        ...
        {showNewModal && (
          <NewDiagramModal onConfirm={handleCreateDiagram} onCancel={() => setShowNewModal(false)} />
        )}
        {folderOps.showFolderModal && (
          <NewFolderModal
            parentName={folderOps.folderModalParentId
              ? folderHook.folders.find(f => f.id === folderOps.folderModalParentId)?.name
              : undefined}
            onConfirm={(name: string, color: FolderColor | null) => folderOps.handleConfirmFolder(name, color)}
        ...
        {folderOps.renameFolderId && (
          <RenameFolderModal
            folder={{
              ...
              name: folderHook.folders.find(f => f.id === folderOps.renameFolderId)?.name || 'Folder',
            }}
