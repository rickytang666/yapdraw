...
export default function ArchitectureDemo() {
  ...
  // 12 beats; last two hold frame
  useEffect(() => {
    const interval = setInterval(() => setStep((s) => (s + 1) % 12), 1000);
    return () => clearInterval(interval);
  }, []);

  // DOM overlay scale = svg viewBox meet math
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    function drawArrow(
      ...
      svgNode.appendChild(
        rc.line(tip[0], tip[1], wing1[0], wing1[1], {
          roughness: 1,
          strokeWidth: 1.8,
          stroke: "#111111",
          seed: seed + 1,
      ...
      svgNode.appendChild(
        rc.line(tip[0], tip[1], wing2[0], wing2[1], {
          roughness: 1,
          strokeWidth: 1.8,
          stroke: "#111111",
          seed: seed + 2,
    ...
    <main className="relative z-10 grid grid-cols-1 lg:grid-cols-[2fr_3fr] mt-2 lg:mt-0 px-4 sm:px-6 lg:px-8 xl:px-10 2xl:px-12 w-full max-w-[1600px] mx-auto min-w-0 shrink-0 lg:min-h-[550px] gap-3 pb-12 max-lg:pb-20 lg:pb-14">
      ...
      >
        ...
        >
          ...
          <div
            className={`absolute top-[40px] left-[320px] w-[100px] h-[70px] bg-white border border-[#E0E0E0] shadow-sm rounded-2xl flex flex-col items-center justify-center gap-1.5 transition-all duration-700 ease-out transform ${step >= 7 ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
          >
            <svg
              viewBox="0 0 24 24"
              className="w-4 h-4"
