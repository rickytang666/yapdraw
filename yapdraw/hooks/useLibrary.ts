...
export function useLibrary() {
  ...
  const diagrams = useMemo(() => {
    ...
    switch (state.activeSection) {
      case 'starred':
        ...
        break
      case 'recent':
        filtered = filtered.sort((a, b) => b.lastOpenedAt - a.lastOpenedAt).slice(0, 20)
        break
      case 'trash':
    ...
    async (ids: string[], tag: string, action: 'add' | 'remove') => {
      ...
      await db.transaction('rw', db.diagrams, async () => {
        for (const d of diagrams) {
          const tags =
            action === 'add' ? [...new Set([...d.tags, tag])] : d.tags.filter((t) => t !== tag)
  ...
  const purgeExpiredTrash = useCallback(async () => {
    ...
    const expired = await db.diagrams
      .filter((d) => d.trashedAt !== null && (d.trashedAt as number) < cutoff)
      .toArray()
