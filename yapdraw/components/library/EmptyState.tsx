...
const CONFIG: Record<Variant, { icon: React.ReactNode; title: string; subtitle: string }> = {
  'empty-library': {
    icon: <IconBooks size={48} className="text-[#94A3B8]" />,
    title: 'No diagrams yet',
    subtitle: 'Create your first diagram to get started.',
  },
  'empty-folder': {
    icon: <IconFolder size={48} className="text-[#94A3B8]" />,
    title: 'This folder is empty',
    subtitle: 'Move or create diagrams here.',
  },
  'no-results': {
    icon: <IconSearchOff size={48} className="text-[#94A3B8]" />,
    title: 'No results found',
    subtitle: 'Try a different search term or filter.',
  },
  'empty-trash': {
    icon: <IconTrashOff size={48} className="text-[#94A3B8]" />,
...
export default function EmptyState({ variant }: Props) {
  ...
  return (
    <div className="flex flex-col items-center justify-center flex-1 gap-3 text-center px-6 select-none">
      ...
      <p className="text-[#475569] font-medium text-base">{title}</p>
      <p className="text-[#94A3B8] text-sm max-w-xs">{subtitle}</p>
    </div>
  )
}
