...
export default function VersionHistoryPanel({ diagramId, isOpen, onClose, canvasRef }: Props) {
  ...
  return (
    <div
      className={`fixed top-0 right-0 h-full z-40 flex flex-col bg-white border-l border-[#E5E7EB] shadow-2xl transition-transform duration-300 ease-in-out ${
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-[#E5E7EB] shrink-0">
        <div className="flex items-center gap-2">
          <IconHistory size={16} className="text-[#64748B]" />
          <h3 className="text-sm font-semibold text-[#0F172A]">Version History</h3>
        </div>
        <button
          onClick={onClose}
          className="p-1 text-[#64748B] hover:text-[#0F172A] transition-colors rounded"
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2">
        {versions.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-3 text-center px-4">
            <IconHistory size={32} className="text-[#D1D5DB]" />
            <p className="text-sm text-[#64748B]">No versions saved yet.</p>
